...
# ==============================================================================
# HTML LOADER & STREAMLIT UI CONFIG
# ==============================================================================

st.set_page_config(
    page_title="Payroll & Attendance System",
    page_icon="📋",
    layout="wide",
...
# Function to read HTML template if available
def load_html():
...
# Dynamic custom CSS integration matching the image design
st.markdown(
    """
    <style>
    ...
    .pill-active { background-color: #E08E0B; color: #FFFFFF; }
    .pill-inactive { background-color: #D9534F; color: #FFFFFF; }
    </style>
""",
...
# Render index.html if user provided it
...
if choice == "👥 Employees Directory":
    ...
    if "view_id" in st.session_state:
        ...
        if not df_emp_v.empty:
            ...
            col2.write(f"**DOJ:** {format_date_display(emp_rec.get('joining_date'))}")
            col2.write(f"**DOE:** {format_date_display(emp_rec.get('doe')) if is_valid_date_str(emp_rec.get('doe')) else 'ACTIVE'}")
            col2.write(f"**TERMINATION DATE:** {format_date_display(emp_rec.get('termination_date')) if is_valid_date_str(emp_rec.get('termination_date')) else 'N/A'}")
    ...
    # EDIT DETAILS
        ...
        if not df_emp_e.empty:
            ...
            with st.form("edit_emp_form_full"):
                ...
                status_options = ["ACTIVE", "INACTIVE", "BLACKLISTED", "TERMINATED"]
                ...
                default_term = datetime.strptime(str(rec.get("termination_date")), "%Y-%m-%d").date() if has_term_date else datetime.now().date()
                u_term_date_val = s_c2.date_input("TERMINATION DATE", value=default_term, format="DD/MM/YYYY", disabled=not term_check)

                has_doe_date = is_valid_date_str(rec.get("doe"))
                doe_check = s_c3.checkbox("SET DATE OF EXIT (DOE)", value=has_doe_date)
                default_doe = datetime.strptime(str(rec.get("doe")), "%Y-%m-%d").date() if has_doe_date else datetime.now().date()
                u_doe_val = s_c3.date_input("DATE OF EXIT (DOE)", value=default_doe, format="DD/MM/YYYY", disabled=not doe_check)
                ...
                u_location = e_c4.text_input("LOCATION", value=str(rec.get("location") if pd.notna(rec.get("location")) else "NEW DELHI").upper()).upper()
                ...
                if btn_save:
                    ...
                    cursor.execute(
                        ...
                        SET name = ?, status = ?, personal_email = ?, office_email = ?, 
                            location = ?, doe = ?, termination_date = ?
                        WHERE emp_id = ?
                        """,
                        (u_name, u_status, u_p_email, u_o_email, u_location, u_doe_parsed, u_term_parsed, ed_id),
                    )
                    conn.commit()
                    conn.close()
    ...
    # DIRECTORY OVERVIEW & TABLE
        ...
        st.markdown('<div class="page-title">Payroll and Attendance System</div>', unsafe_allow_html=True)
        st.markdown('<div class="breadcrumb">🏠 Home &nbsp;•&nbsp; Attendance</div>', unsafe_allow_html=True)
        ...
        with ctrl_col3:
            search_query = st.text_input("Search:", "", placeholder="Search...").strip().upper()
        ...
        if df_all_emp.empty:
            ...
            h_c5.markdown("**Tools**")
            st.markdown("<hr style='margin: 8px 0px; border-color:#E5E5E5;'>", unsafe_allow_html=True)
            ...
            for _, row in filtered_df.iterrows():
                ...
                pill_class = "pill-active" if st_status == "ACTIVE" else "pill-inactive"
                r_c4.markdown(f'<span class="pill-status {pill_class}">{st_status.lower()}</span>', unsafe_allow_html=True)
                ...
                st.markdown("<hr style='margin: 6px 0px; border-color:#F0F0F0;'>", unsafe_allow_html=True)

elif choice == "📅 Attendance Hub":
    st.markdown('<div class="page-title">📅 Attendance Hub</div>', unsafe_allow_html=True)
    st.info("Attendance hub dashboard active.")

elif choice == "➕ Onboard Employee":
    st.markdown('<div class="page-title">➕ Onboard Employee</div>', unsafe_allow_html=True)
    st.info("Onboarding wizard active.")

elif choice == "🌴 Leave Portal":
    st.markdown('<div class="page-title">🌴 Leave Portal</div>', unsafe_allow_html=True)
