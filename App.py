...
if st.session_state["active_tab"] == "Manage Employees":
    ...
    db_depts = [str(x).upper() for x in df_all_emp["department"].dropna().unique() if str(x).strip() != ""] if not df_all_emp.empty else []
    all_depts_unique = sorted(list(set(STANDARD_DEPTS + db_depts)))

    db_locs = [str(x).upper() for x in df_all_emp["location"].dropna().unique() if str(x).strip() != ""] if not df_all_emp.empty else []
...
# ==============================================================================
# MODALS & POPUPS (ADD EMPLOYEE / VIEW / EDIT / DELETE)
# ==============================================================================
